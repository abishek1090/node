...
const Comment = ({ _id, message, username, createdAt }) => {
  ...
  const handleDelete = async () => {
    try {
      await axios.post(`http://localhost:8080/api/comment/delete`, { id: params.id, _id: _id });
      // console.log(_id);
      // const exportFilterDashboard = ;
      // console.log(exportFilterDashboard);
      setAvailableComments((prev)=>[availableComments.push({}),...prev]);
    }
    catch (err) {
      console.log(err);
    }
